...
A user token encodes a reference (a URI identifying the invited user), and a resource that contains that reference.  The reference is typically a `mailto:` uri containing a name and e-mail address, but that is neither required nor relevant to this libary.
...
3. When given a user token of the form of a URL parameter `userToken=_THE_TOKEN_`, will give the bearer of the token permissions to the submission specified in the token.

For new users invited via a user token, the token encodes the identity of a submission (its URI), and the identity of a user, which is the user's e-mail addres.  If the e-mail URI present in `submission.getSubmitterEmail()`) matches the token, and the submission identity matches the token, then the newly-created User will become the submission's submitter, and the `submitterEmail` and `submitterName` fields will be nulled out.
